...
<template>
  <div class="px-3 py-3 dark:bg-[#0D0D0D99] dark:text-white rounded-lg">
    <div v-if="getUserType === 'company'">
      <section
        class="flex flex-col md:flex-row justify-between md:justify-start gap-2 md:items-center my-1 mt-0"
      >
        <p class="dark:text-gray-200 ml-3">
          {{ companySummary.totalUsers }} users have joined your facility
        </p>
      ...
      <section class="my-6" > 
        <UAlert
          v-if="
            getUserType === 'company' &&
            companySummary.subscriptionsToAttendTo > 0
          "
          class="mb-2 border border-blue-500 dark:border-gray-600"
          :close-button="{
            color: 'orange',
            variant: 'outline',
            padded: false,
        ...
        />
        <h2
          class="text-lg text-gray-600 dark:text-gray-200 font-semibold mb-5 mt-5 ml-3"
        >
          Analytics
        </h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 ml-3">
          <SummaryCard title="Total users" :value="companySummary.totalUsers" />
          <SummaryCard
            title="Active Subscriptions"
            :value="companySummary.totalActiveSubscriptions"
          />
          <SummaryCard
            title="Available Services"
            :value="companySummary.totalServiceCount"
          />
        </div>
      </section>
      ...
      <section class="mb-10 items-center justify-left">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-7 ml-3 text-left">
          <QuickAction
            title="Invite Users"
            :icon="greenPlusPath"
            :action="() => navigateTo('/dashboard/users?openInviteForm=yes')"
            class="py-4 px-2 w-full dark:bg-gray-700 dark:hover:bg-gray-600"
          />
          <QuickAction
            title="View Services"
            :icon="orangeBagPath"
          ...
          <QuickAction
            ...
            class="py-4 w-full dark:bg-gray-700 dark:hover:bg-gray-600"
          />
        </div>
      </section>
    </div>
      ...
      <section class="mb-10">
        ...
        <div v-if="userSubscriptionList.length > 0">
          <div class="flex flex-col lg:flex-row gap-2 mb-2">
            <ActiveSubscriptionCard
              v-for="sub in userSubscriptionList"
              :company-name="sub.company.name"
              :service-name="sub.actualSubscriptionName"
              :start-date="formatDateAddDay(sub.startDate)"
        ...
        <div v-else class="text-center">
          ...
          <UButton
            color="cyan"
            variant="outline"
            @click="navigateTo('/dashboard/joined-centers')"
